...
/// </summary>
internal sealed class SixelSvgImage : Renderable, IDisposable
{
    /// <summary>
    ...
    public SixelSvgImage(Stream data)
    {
        using SKSvg svg = SKSvg.CreateFromStream(data);

        SKRect svgSize = svg.Picture?.CullRect ?? SKRect.Empty;

        Image = new SKBitmap((int)svgSize.Width, (int)svgSize.Height);
